<script lang="ts" options accessors={true}>
...
`
	...
	function toSvgPath(qr: qrcodegen.QrCode, border: number, lightColor: string, darkColor: string): string {
		if (border < 0)
			throw new RangeError("Border must be non-negative");
		let parts: Array<string> = [];
		for (let y = 0; y < qr.size; y++) {
	...
	let c = {
		...
		text:'',
		timer:false,
		icon:'https://cdn.rutynka.io/img/icon/btc.svg'
	}
	export const set = function set(obj) {
		Object.assign(c, obj)
		const QRC = qrcodegen.QrCode;
		const qr0 = QRC.encodeText(c.address, QRC.Ecc.MEDIUM);
		c.size = qr0.size
		qrcode = toSvgPath(qr0, 4, '#000', '#fff');  // See qrcodegen-input-demo
		if (obj.timer) {
			setTimeout(() => {
				c.show = false
				c.timer = false
	...
	// Simple operation
	// const svg = toSvgString(qr0, 4, '#000', '#fff');  // See qrcodegen-input-demo
...
<qr-crypto>
	...
	{#if c.address}
		<img on:click={() => {c.show = !c.show}} alt="cryptocurrency icon" src='{c.icon}'>
		<div>
			{#if c.show}
				{#if c.text}
					<h4>{c.text}</h4>
				{/if}
				<svg xmlns="http://www.w3.org/2000/svg" version="1.1" viewBox="3 3 {(c.size + c.border * 2)} {(c.size + c.border * 2)}" stroke="none">
					<rect width="100%" height="100%" fill="{c.lightColor}"/>
					<path d="{qrcode}" fill="{c.darkColor}"/>
				</svg>
				{#if c.balance}
					<div class='balance'>
						<p>{c.balance}</p>
...
<style>
	qr-crypto{font-size:1rem; display: flex;justify-content: center;text-align: center;}
	img {cursor: pointer;width:32px;margin:0}
	p {text-align: center;font-size:1em;word-break: break-all;word-wrap: anywhere;}
	svg {width: 200px;}
